...
'''
mm = '''
قم بلأنضمام الى قناة 𝗟𝗘𝗮𝗗𝗲𝗥@V_P_N_8
'''
...
keyboard = [
  ...
  [
    Button.url("سورس 𝗟𝗘𝗮𝗗𝗲𝗥🤡", "https://t.me/V_P_N_8")
    ]
]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply(i + "\n\nشكرا لأستخدامك سورس 𝗟𝗘𝗮𝗗𝗲𝗥", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply("لقد تم حذف القناة/الكروب شكرا لأستخدامك 𝗟𝗘𝗮𝗗𝗲𝗥.", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply("لقد تم انهاء جميع الجلسات شكراً لأستخدامك 𝗟𝗘𝗮𝗗𝗲𝗥.", buttons=keyboard)
