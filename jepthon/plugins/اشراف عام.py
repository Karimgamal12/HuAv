                                 ...
                                 ChatAdminRights,
                                   ChatBannedRights,
                                     MessageEntityMentionName,
                                       MessageMediaPhoto)
from sourceav.utils import admin_cmd
from ..Config import Config
from sourceav import CMD_HELP, jepiq
...
async def get_full_user(event):  
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        try:
            ...
            return await event.edit("▾∮ هنالك خطأ يرجى تبليغنا @sourceav", str(err))           
...
async def gben(sourceav):
    dc = razan = sourceav
    i = 0
    sender = await dc.get_sender()
    me = await sourceav.client.get_me()
    await razan.edit("▾∮ يتم رفع المستخدم في جميع المجموعات")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await sourceav.get_chat()
    if sourceav.is_private:
        user = sourceav.chat
        rank = sourceav.pattern_match.group(1)
    else:
        sourceav.chat.title
    try:
        user, rank = await get_full_user(sourceav)
    except:
        pass
    if me == user:
    ...
    if user:
        telchanel = [d.entity.id
                     for d in await sourceav.client.get_dialogs()
        ...
        for x in telchanel:
          try:
             await sourceav.client(EditAdminRequest(x, user, rgt, rank))
             i += 1
