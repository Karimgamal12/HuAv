...
from V_P_N_8 import BOTLOG, BOTLOG_CHATID, PM_LOGGER_GROUP_ID
...
from flask import Flask
 ...
 if __name__ == '__main__':
     app.run()
LOGS = logging.getLogger("V_P_N_8")
...
async def startupmessage():
    ...
    try:
        if BOTLOG:
            Config.CATUBLOGO = await jepiq.tgbot.send_file(
                ...
                caption="**᯽︙ بــوت 𝗟𝗘𝗮𝗗𝗲𝗥يـعـمـل بـنـجـاح ✓ **\n**᯽︙ ارسل `.الاوامر` لرؤية اوامر السورس**",
                buttons=[(Button.url("سورس اݪقائد", "https://t.me/V_P_N_8"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
    ...
    try:
        if msg_details:
            ...
            text = (
                message.text
                + "\n\n**᯽︙اهلا وسهلا لقد قمت باعاده تشغيل بـوت 𝗟𝗘𝗮𝗗𝗲𝗥تمت بنجاح**"
            )
...
async def add_bot_to_logger_group(chat_id):
    ...
    try:
        ...
        try:
            ...
            LOGS.error(str(e))
#by @V_P_N_8 بس اشوفك خامطه للكود اهينك وافضحك 

V_P_N_8 = {"@V_P_N_8", "@Help_MonZer_7_P", "@Help_Tele_Av"}
async def saves():
   for lMl10l in V_P_N_8:
        try:
...
async def load_plugins(folder, extfolder=None):
    ...
    if extfolder:
        ...
        path = f"V_P_N_8/{folder}/*.py"
        plugin_path = f"V_P_N_8/{folder}"
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        descript = "- عزيزي المستخدم هذه هي مجموعه الاشعارات يرجى عدم حذفها  - @V_P_N_8"
        photobt = await jepiq.upload_file(file="JepIQ/razan/resources/start/Al.jpg")
        _, groupid = await create_supergroup(
            "مجموعة أشعارات 𝗟𝗘𝗮𝗗𝗲𝗥", jepiq, Config.TG_BOT_USERNAME, descript, photobt
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        ...
        photobt = await jepiq.upload_file(file="JepIQ/razan/resources/start/Le.jpg")
        _, groupid = await create_supergroup(
...
async def install_externalrepo(repo, branch, cfolder):
    ...
    if V_P_N_8BRANCH := branch:
        repourl = os.path.join(V_P_N_8REPO, f"tree/{V_P_N_8BRANCH}")
        gcmd = f"git clone -b {V_P_N_8BRANCH} {V_P_N_8REPO} {cfolder}"
        errtext = f"لا يوحد فرع بأسم `{V_P_N_8BRANCH}` في الريبو الخارجي {V_P_N_8REPO}. تاكد من اسم الفرع عبر فار (`EXTERNAL_REPO_BRANCH`)"
    else:
        repourl = V_P_N_8REPO
        gcmd = f"git clone {V_P_N_8REPO} {cfolder}"
        errtext = f"الرابط ({V_P_N_8REPO}) الذي وضعته لفار `EXTERNAL_REPO` غير صحيح عليك وضع رابط صحيح"
    ...
    await load_plugins(folder="V_P_N_8", extfolder=cfolder)
